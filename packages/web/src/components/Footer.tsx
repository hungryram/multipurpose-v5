...
export default async function Footer() {
  ...
  const textColor = appearance?.footer?.textColor?.hex
  const headingColor = appearance?.footer?.headingColor?.hex || textColor
    ...
    >
      <div className="container mx-auto px-4 py-12">
        <div className="grid gap-8 md:grid-cols-3">
          <div>
            <h3 className="mb-4 text-lg font-semibold" style={{color: headingColor || undefined}}>{profile?.company_name}</h3>
            {appearance?.footer?.footerText && (
              <div className="text-sm opacity-80" style={{color: textColor || undefined}}>
          ...
          {profile?.contact_information && (
            <div>
              <h3 className="mb-4 text-lg font-semibold" style={{color: headingColor || undefined}}>Contact</h3>
              <div className="space-y-2 text-sm opacity-80" style={{color: textColor || undefined}}>
                {profile.contact_information.email && (
                  <p style={{color: textColor || undefined}}>Email: {profile.contact_information.email}</p>
                )}
                {profile.contact_information.phone && (
                  <p style={{color: textColor || undefined}}>Phone: {profile.contact_information.phone}</p>
                )}
              </div>
            </div>
          )}
          ...
          {profile?.social && (
            <div>
              <h3 className="mb-4 text-lg font-semibold" style={{color: headingColor || undefined}}>Follow Us</h3>
              <div className="flex space-x-4" style={{color: textColor || undefined}}>
